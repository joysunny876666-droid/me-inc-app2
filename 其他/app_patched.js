    ...
    els.addForm.inputs.isRecurringRadios.forEach(radio => {
        radio.addEventListener('change', (e) => {
            ...
            if (e.target.value === 'yes') {
                ...
                if (dateFieldLabel) dateFieldLabel.textContent = '?��??��?';
...
function updateRecurringSingle() {
    ...
    // 將這次?�次修改作為 Exception ?�件存入，�??�創建新任�???+    // 如�?修改了日?��?必�??��??��?標�???true (?�除)，並?�新?��?寫入?�件
    if (editPendingData.newDate !== editOriginalDateVal) {
        ...
        // 轉移該單次任?��?完�?紀??+        if (freshTask.completedHistory && freshTask.completedHistory[editOriginalDateVal]) {
...
function updateRecurringFuture() {
    ...
    // 繼承歷史紀??+    const inheritedCompleted = {};
...
/**
 * Returns a formatted recurrence display string: "M/D | �?N ??| 下次: M/D"
 */
function getRecurrenceDisplayInfo(task, currentDateStr) {
    ...
    const intervalLabel = `�?${interval} ${typeLabel}`;
...
function renderTodayFlowchart() {
    ...
    if (dateLabel) {
        ...
        dateLabel.textContent = `今日待辦 (${d.getMonth() + 1}??{d.getDate()}??- ??${untimedTasks.length} ??`;
    }
    ...
    if (chipsContainer) {
        ...
        if (untimedTasks.length === 0) {
            chipsContainer.innerHTML = '<span style="color:var(--text-secondary);font-size:0.8rem;">?��?辦�???/span>';
        } else {
